...
def to_batch_tables1( tables, B, table_type):
    # col_lens = []
    col_seq = []
...
class SuperModel(Model):
    def __init__(self, word_emb, N_word, N_h=300, N_depth=2, gpu=True, trainable_emb=False, table_type="std", use_hs=True):
        ...
        self.SQL_OPS = ('none','intersect', 'union', 'except')
        self.KW_OPS = ('where','groupBy','orderBy')
        self.AGG_OPS = ('max', 'min', 'count', 'sum', 'avg')
        self.ROOT_TERM_OPS = ("root","terminal")
        self.COND_OPS = ("and","or")
        self.DEC_ASC_OPS = (("asc",True),("asc",False),("desc",True),("desc",False))
        self.NEW_WHERE_OPS = ('=','>','<','>=','<=','!=','like','not in','in','between')
        self.KW_WITH_COL = ("select","where","groupBy","orderBy","having")
        self.table_dict = get_table_dict(PATH_TO_TABLES)
    ...
    def forward(self, input_dict):
        ...
        # else:
        # print(input_dict)
        sql= self.full_forward([input_dict.fields["question_toks"]],[],self.table_dict[input_dict.fields["db_id"]])
        # print (sql)
        if sql is not None:
            print(sql)
            sql = self.gen_sql(sql,self.table_dict[input_dict.fields["db_id"]])
        else:
            sql = "select a from b"
        return sql
    ...
    def full_forward(self, q_seq, history, tables):
        ...
        col_seq = to_batch_tables1(tables, B, self.table_type)
        ...
        while not stack.isEmpty():
            ...
            if isinstance(vet,tuple) and vet[0] == "root":
                ...
                # andor_cond = self.COND_OPS[label]
                # history.append("")
            # elif vet == "groupBy":
            #     score = self.having.forward(q_emb_var,q_len,hs_emb_var,hs_len,col_emb_var,col_len,)
                ...
                # print("agg:{}".format([self.AGG_OPS[agg] for agg in agg_idxs]))
                if len(agg_idxs) > 0:
                    history[0].append(self.AGG_OPS[agg_idxs[0]])
                    if vet[1] not in ("having", "orderBy"):
                        current_sql[kw].append(self.AGG_OPS[agg_idxs[0]])
                    elif vet[1] == "orderBy":
                        stack.push(("des_asc", vet[2], self.AGG_OPS[agg_idxs[0]])) #DEBUG-ed 20180817
                    else:
                        stack.push(("op","having",vet[2],self.AGG_OPS[agg_idxs[0]]))
                for agg in agg_idxs[1:]:
                    history[0].append(index_to_column_name(vet[2], tables))
                    history[0].append(self.AGG_OPS[agg])
                    if vet[1] not in ("having", "orderBy"):
                        current_sql[kw].append(index_to_column_name(vet[2], tables))
                        current_sql[kw].append(self.AGG_OPS[agg])
                    elif vet[1] == "orderBy":
                        stack.push(("des_asc", vet[2], self.AGG_OPS[agg]))
                    else:
                ...
                # current_sql[kw].append([self.AGG_OPS[agg] for agg in agg_idxs])
                # if vet[1] == "having":
                #     stack.push(("op","having",vet[2],agg_idxs))
                # if vet[1] == "orderBy":
                ...
                # current_sql[kw].append([self.NEW_WHERE_OPS[op] for op in ops])
                if op_num > 0:
                    history[0].append(self.NEW_WHERE_OPS[ops[0]])
                    if vet[1] == "having":
                        stack.push(("root_teminal", vet[2],vet[3],ops[0]))
                    else:
                        stack.push(("root_teminal", vet[2],ops[0]))
                    # current_sql[kw].append(self.NEW_WHERE_OPS[ops[0]])
                for op in ops[1:]:
                    history[0].append(index_to_column_name(vet[2], tables))
                    history[0].append(self.NEW_WHERE_OPS[op])
                    # current_sql[kw].append(index_to_column_name(vet[2], tables))
                    # current_sql[kw].append(self.NEW_WHERE_OPS[op])
                    if vet[1] == "having":
                ...
                if len(vet) == 4:
                    ...
                    try:
                        ...
                        print("sql_stack:{}".format(sql_stack))
                        exit(1)
                    current_sql[kw].append(self.NEW_WHERE_OPS[vet[2]])
                if label == "root":
                    history[0].append("root")
                ...
                label = np.argmax(score[0].data.cpu().numpy())
                dec_asc,has_limit = self.DEC_ASC_OPS[label]
    ...
    def gen_sql(self, sql,table):
        ...
        if "nested_label" in sql:
            ...
            sql = sql["sql"]
        for key in sql:
            if key not in self.KW_WITH_COL:
                continue
            for item in sql[key]:
...
#newly added methods
    ...
    def _load(cls,
        ...
        def get_model():
            ...
            word_emb = load_word_emb('syntaxsql/glove.%dB.%dd.txt'%(B_word,N_word), \
                    load_used=None, use_small=USE_SMALL)
            ...
            model = SuperModel(word_emb, N_word=N_word, gpu=GPU, trainable_emb = None, table_type="std", use_hs=True)
            return model

        model=get_model()
        with open("syntaxsql/model.th", 'rb') as f:
            print("loading model")
